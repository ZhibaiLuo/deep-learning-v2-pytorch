...
class NeuralNetwork(object):
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        print('error:')
        print(y.shape, final_outputs.shape, error.shape)
        ...
        # 这两点更新其实是(n, 1), (1, k)的矩阵乘法，这个broadcasting规则比较奇怪
#         delta_weights_i_h += self.lr * X[:,None].dot(hidden_error_term[None,:])
...
#         delta_weights_h_o += self.lr * hidden_outputs[:,None].dot(output_error_term[None,:])
    ...
    def run(self, features):
        ...
        ### Forward pass ###
        # TODO: Hidden layer - Replace these values with your calculations.
        hidden_inputs = np.array(features).dot(self.weights_input_to_hidden) # signals into hidden layer
        hidden_outputs = self.activation_function(hidden_inputs) # signals from hidden layer
